 ...
 *	var JiliangLeibieStore = [['', '-显示所有-'],['001', '质量'],['002', '电力']……]
 * 
 * 
 * 用法2不带name用法
 *  * <lilydap:commonExtSimpleStore model="energy.JiliangLeibie" valueField="code" displayField="name" selectText="-显示所有-" extendText="key:01,value:测试~key:02,value:测试2..."/>
 * 执行后将生成类似于如下代码：fields: ['dictCode', 'dictValue']之所以用和字典表一样的字段是因为在前台可以采用现有字典表的所有功能，而不用再写。
 *	[['', '-显示所有-'],['001', '质量'],['002', '电力']……]
 * 
 * @author 孙成才
 *
 * @jsp.tag name="commonExtFields"
 */
public class CommonExtFieldsTag extends TagSupport {
	/**
	 * 
	 */
	...
	public int doStartTag() throws JspException {		
    	...
    	if (!"".equals(extendText)) {
    		...
    		for(int i = 0; i < textArr.length; i++){
    			...
    			if(i == 0){
    				buf.append("['" + text[0].split(":")[1] + "', '" + text[1].split(":")[1] + "']"); 
    			}else{
    				buf.append(",['" + text[0].split(":")[1] + "', '" + text[1].split(":")[1] + "']"); 
    			}    		
    		}
