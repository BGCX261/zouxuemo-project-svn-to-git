...
public class WordOperate {
	...
	public void generate(OutputStream result, String fileName, Table table, Map<String, CssClass> cssClassMap, Map<String, String> config) throws IOException, DocumentException {
		...
		RtfWriter2.getInstance(document, result);
		document.open();
		...
		for (Row row : table.getData()) {
			for (Cell cell : row.getData()) {
				com.lowagie.text.Cell textCell = createCell(cssClassMap, table, row, cell);
	...
	private com.lowagie.text.Cell createCell(Map<String, CssClass> cssClassMap, Table table, Row row, Cell cell) throws DocumentException, IOException {
...
//			img.scaleAbsolute(chartWidth, chartHeight);
//			img.scalePercent(chartConfig.getWidth(), chartConfig.getHeight());// 表示显示的大小为原尺寸的50%
	...
	/*public static void main(String[] args) throws IOException, DocumentException {
		...
		//-----------------------------------------------------------------
    	FileOutputStream fo = new FileOutputStream("./example.doc");
