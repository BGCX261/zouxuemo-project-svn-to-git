...
//	@Rollback(false)
	...
	public void testQueryClassOfTCondition() throws ParseException {
		...
		list = dao.query(Module.class, Condition.create().llike("code", "code").ne("code", "code"));
	...
	public void testStat() {
		...
		Assert.assertEquals("code9", dao.max(Module.class, "code", null));
		Assert.assertEquals(null, dao.max(Module.class, "code", Condition.create().eq("code", "not exist")));
	}
...
//		int count = dao.execute("insert into Module (code, name) select code, ? from Module where code in (?, ?)", "HQL", "code2", "code3");
	...
	public void testSqlExecute() {
		...
		Condition condition = Condition.create().in("code", "code2", "code3");
		dao.sqlExecute("insert into module (code, name, default_module, create_date, num, cnt) select code, '修改的名称', default_module, create_date, num, cnt from module", condition);
	...
	public void testSqlQuery() {
		...
		Condition condition = Condition.create().llike("module.code", "code1").desc("module.code").limit(2, 0);
		List<Map<String, Object>> list3 = dao.sqlQuery("select id, code, name, num, create_date, default_module, des from module where num > 0", condition);
		Assert.assertEquals(2, list3.size());
		Assert.assertEquals("code10", list3.get(0).get("code"));
	}
	
	@Test
	public void testSqlCount() {
		long count = dao.sqlCount("select id, code, name, num, create_date, default_module from module");
		Assert.assertEquals(13, count);
		
		count = dao.sqlCount("select id, code, name, num, create_date, default_module, des from module where code like 'code1%'");
		Assert.assertEquals(4, count);
		
		Condition condition = Condition.create().llike("module.code", "code1").desc("module.code").limit(2, 0);
		count = dao.sqlCount("select id, code, name, num, create_date, default_module, des from module where num > 0", condition);
		Assert.assertEquals(4, count);
	}
}
